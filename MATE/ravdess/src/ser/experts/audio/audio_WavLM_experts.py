...
class AudioUniversalNpyExpert(nn.Module):
    """
    统一吃帧级 npy 特征的音频专家（WavLM/Whisper 都能用）
    batch keys:
      - x_audio: [B,T,D]
      - x_audio_mask: [B,T] (1 valid, 0 pad)  (你的 AttentionPooling 已经按这个mask逻辑写了) :contentReference[oaicite:5]{index=5}
    """
    ...
    def forward_with_extras(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        out = self.model(x, mask)  # out={'logits','pooled'} :contentReference[oaicite:6]{index=6}
        logits = out["logits"]
        pooled = out["pooled"]

        # 让 analyze_expert.py 也能拿到 attn（否则就给一个 dummy，全流程不炸）
