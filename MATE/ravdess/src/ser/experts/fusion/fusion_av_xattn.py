...
"""Audio-Visual fusion expert: Cross-Attention + Shared Transformer + Attentive Stats Pooling.

This is a *strong, standard* frozen-feature fusion architecture:
  1) per-modality projection -> modality-specific transformer encoder
  2) bi-directional cross-attention (A->V, V->A) with residual
  3) token-level shared transformer over concatenated (A,V) tokens
  4) attentive statistics pooling + linear classifier

It fits your repo contracts:
  - expert.type starts with "fusion_" so scripts/train.py will dispatch to fusion dataloader
  - forward(batch)->logits [B,C]
  - forward_with_extras(batch)->(logits, pooled, attn) for analyze_expert.py

Batch keys (from collate_fusion_av_npy):
  x_audio, x_audio_mask, x_video, x_video_mask, y
"""
...
@register_expert("fusion_av_xattn")
class FusionAVCrossAttnExpert(nn.Module):
    """Config keys (expert):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # --- cross-attention ---
        self.xattn_a2v = nn.MultiheadAttention(self.h, self.nhead, dropout=self.dropout, batch_first=True)
        self.xattn_v2a = nn.MultiheadAttention(self.h, self.nhead, dropout=self.dropout, batch_first=True)
        self.xattn_drop = nn.Dropout(self.dropout)
        self.xattn_ln_a = nn.LayerNorm(self.h)
        self.xattn_ln_v = nn.LayerNorm(self.h)

        # --- modality embeddings (for shared encoder after concat) ---
        if self.use_mod_emb:
    ...
    def forward_with_extras(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 2) bi-directional cross-attention + residual
        #    A attends to V
        a2v, _ = self.xattn_a2v(query=ha, key=hv, value=hv, key_padding_mask=_key_padding(mv))
        ha = self.xattn_ln_a(ha + self.xattn_drop(a2v))
        if ma is not None:
            ha = ha.masked_fill(ma.unsqueeze(-1) == 0, 0.0)

        #    V attends to A
        v2a, _ = self.xattn_v2a(query=hv, key=ha, value=ha, key_padding_mask=_key_padding(ma))
        hv = self.xattn_ln_v(hv + self.xattn_drop(v2a))
        if mv is not None:
        ...
        x_cat = torch.cat([ha, hv], dim=1)
        if ma is None and mv is None:
            m_cat = None
        else:
            if ma is None:
                ma = xa.new_ones((xa.shape[0], xa.shape[1]), dtype=torch.long)
            if mv is None:
                mv = xv.new_ones((xv.shape[0], xv.shape[1]), dtype=torch.long)
            m_cat = torch.cat([ma, mv], dim=1)
