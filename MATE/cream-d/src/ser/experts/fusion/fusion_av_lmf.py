...
"""
Audio-Visual fusion expert: LMF (Low-rank Multimodal Fusion) over pooled representations.
...
Pipeline:
  ...
  3) Low-rank multiplicative fusion (LMF)
  4) classifier
...
forward_with_extras(batch) -> (logits, pooled, attn_cat)
  ...
  - attn_cat: concatenated pooling attn [B, Ta_max + Tv_max]
"""
...
def _pad_attn(attn: torch.Tensor, mask: Optional[torch.Tensor], T_max: int) -> torch.Tensor:
    """
    attn: [B,T], mask: [B,T] or None
    returns: [B,T_max] padded with 0
    """
    B, T = attn.shape
    if T == T_max:
        out = attn
        ...
        out[:, :T] = attn
    if mask is not None:
        # make sure padded positions are 0
        if T < T_max:
            out[:, T:] = 0.0
    return out
...
class LowRankFusion(nn.Module):
    ...
    where Wi[r] maps (in_dim+1) -> out_dim.

    Here we implement for m=2 (audio+video), but code supports generic list.
    """
...
@register_expert("fusion_av_lmf")
class FusionAVLMFExpert(nn.Module):
    """
    Config keys (expert):
    ...
    Notes:
      - LMF runs on pooled stats vectors: (2H audio) + (2H video)
    """
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # modality encoders (cheap, optional)
    ...
    def forward_with_extras(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        fused = self.fuse((pa, pv))       # [B,F]
        logits = self.cls(fused)

        # build a concatenated attention for analyze: [B, Ta_max+Tv_max]
        Ta_max = xa.shape[1]
        Tv_max = xv.shape[1]
        attn_cat = torch.cat([
            _pad_attn(attn_a, ma, Ta_max),
            _pad_attn(attn_v, mv, Tv_max),
        ], dim=1)
