...
【文件作用】校准算法与评估工具箱
包含：
1. Logit Calibrators: 调整 Logits 分布 (Temperature Scaling, Vector Scaling)
2. Confidence Calibrators: 调整置信度标量 (Isotonic, Linear) -> 用于 MoCaE 路由
3. Metrics: ECE, NLL, Brier Score 计算
4. Tools: Reliability Diagram (可靠性曲线) 数据生成

主要用于解决模型“盲目自信”问题，即解决“庸医”问题，使输出概率与真实准确率对齐。
...
# NOTE:
# - "Logit calibrators" (temperature/vector): 修改 Logits 向量，进而改变 Softmax 后的整个概率分布。
#   用于优化单个专家的分类性能（NLL/ECE）。
#
# - "Confidence calibrators" (isotonic/linear): 将标量置信度 (Max Probability) 映射为 -> 校准后的置信度 (P(correct))。
#   这是 MoCaE (Mixture of Calibrated Experts) 的核心：先让所有专家的置信度在同一个“真实性”量纲上对齐，
#   然后再进行 Gating（门控）或 Routing（路由），防止高置信度的“庸医”主导结果。
...
class TemperatureScaling(Calibrator):
    ...
    def fit(self, logits: np.ndarray, y_true: np.ndarray) -> "TemperatureScaling":
        ...
        def closure():
            ...
            loss = F.cross_entropy(logits_t / T, y_t)  # 优化目标：交叉熵 (即 NLL)
            loss.backward()
            return loss

        # 循环多次调用 step (因为 LBFGS 内部可能有多次迭代)
...
class VectorScaling(Calibrator):
    ...
    def __init__(self, num_classes: int, max_iter: int = 3000, lr: float = 1e-2, weight_decay: float = 1e-4,
        ...
        self.max_iter = int(max_iter)
        self.lr = float(lr)
        self.weight_decay = float(weight_decay)  # 权重衰减很重要，防止过拟合
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
...
class ConfidenceIsotonic(Calibrator):
    ...
    def fit(self, logits: np.ndarray, y_true: np.ndarray) -> "ConfidenceIsotonic":
        ...
        p = _softmax(logits)
        conf = p.max(axis=1).astype(np.float64)  # 原始置信度
        pred = p.argmax(axis=1)
        correct = (pred == y_true).astype(np.float64)  # 0 或 1

        # 拟合 Confidence -> Correctness 的映射
        ir = IsotonicRegression(y_min=0.0, y_max=1.0, increasing=True, out_of_bounds="clip")
        ir.fit(conf, correct)

        # 保存阈值用于推理 (numpy interpolate)
...
# 说明：这些是校准评估指标与 reliability diagram 计算工具，供 scripts/calibrate.py 使用。
...
def ece_from_confidence(conf: np.ndarray, correct01: np.ndarray, n_bins: int = 15) -> float:
    ...
    for i in range(n_bins):
        ...
        if not np.any(m):
            continue
        acc = float(correct01[m].mean())  # 该 bin 内的真实准确率
        avg_conf = float(conf[m].mean())  # 该 bin 内的平均置信度
        ece += (m.sum() / N) * abs(acc - avg_conf)  # 加权绝对误差
    return float(ece)
...
def ece_multiclass(logits: np.ndarray, y_true: np.ndarray, n_bins: int = 15) -> float:
    ...
    p = softmax(logits)
    conf = p.max(axis=1)  # 预测置信度
    pred = p.argmax(axis=1)
    correct = (pred == y_true).astype(np.float32)  # 是否预测正确
...
def reliability_curve_from_confidence(conf: np.ndarray, correct01: np.ndarray, n_bins: int = 15) -> Tuple[
    ...
    for i in range(n_bins):
        ...
        if not np.any(m):
            continue
        centers.append((lo + hi) / 2)  # Bin 的中心点
        accs.append(float(correct01[m].mean()))  # Y轴: 实际准确率
        confs.append(float(conf[m].mean()))  # (可选) 实际的平均置信度
