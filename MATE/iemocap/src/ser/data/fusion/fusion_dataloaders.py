...
from .fusion_avt_npy import FusionAVTNpyDataset, collate_fusion_avt_npy
...
def make_fusion_loaders(
    ...
    Currently supports:
      ...
      - fusion_avt_* : audio + video + text frame/token-level features
    """
    e = cfg.get("expert", {}) or {}

    is_avt = str(expert_type).startswith("fusion_avt_")

    audio_feat_root = e.get("audio_feat_root")
    video_feat_root = e.get("video_feat_root")
    text_feat_root = e.get("text_feat_root") if is_avt else None

    if is_avt:
        if audio_feat_root is None or video_feat_root is None or text_feat_root is None:
            raise KeyError(
                f"text_feat_root in expert.*."
                "Example: expert.audio_feat_root, expert.video_feat_root, expert.text_feat_root"
            )
    else:
    ...
    text_ext = str(e.get("text_ext", e.get("ext_text", ".npy"))) if is_avt else None
    text_feat_key = str(e.get("text_feat_key", e.get("feat_key_text", "x"))) if is_avt else None

    audio_input_dim = e.get("audio_input_dim")
    video_input_dim = e.get("video_input_dim")
    text_input_dim = e.get("text_input_dim") if is_avt else None
    ...
    text_input_dim = int(text_input_dim) if text_input_dim is not None else None
    ...
    max_frames_text = e.get("max_frames_text", 128) if is_avt else None
    ...
    max_frames_text = int(max_frames_text) if max_frames_text is not None else None

    allow_missing_audio = bool(e.get("allow_missing_audio", False))
    allow_missing_video = bool(e.get("allow_missing_video", False))
    allow_missing_text = bool(e.get("allow_missing_text", False)) if is_avt else False

    if is_avt:
        ds_tr = FusionAVTNpyDataset(
            items_train,
            label2id=label2id,
            audio_feat_root=str(audio_feat_root),
            video_feat_root=str(video_feat_root),
            text_feat_root=str(text_feat_root),
            audio_ext=audio_ext,
            video_ext=video_ext,
            text_ext=str(text_ext),
            video_feat_key=video_feat_key,
            text_feat_key=str(text_feat_key),
