...
def _get_item_label(it: Dict[str, Any]):
    lb = it.get("label", it.get("emotion", None))
    if lb is None:
...
def _infer_numeric_label_mode(items: List[Dict[str, Any]], num_classes: int, scan_n: int = 2000) -> Tuple[bool, int]:
    """
    Infer whether manifest labels are numeric.
    Returns: (is_numeric, offset)
      - is_numeric=False: treat labels as strings.
      - is_numeric=True：
          offset=0: labels are 0-based (0..C-1).
          offset=-1: labels are 1-based (1..C).
    Only infer 1-based labels when the evidence is clear.
    """
    vals: List[int] = []
...
def _build_label2id(labels: List[Any], num_classes: int, numeric: bool, offset: int) -> Dict[Any, int]:
    """
    Build a more robust label2id mapping:
    - Always include string labels from the config.
    - Also support numeric labels from the manifest when present.
        offset=0 :  '0'..'C-1' / 0..C-1
        offset=-1: map '1'..'C' or 1..C to 0..C-1
    """
...
def _label_to_id(lb: Any, label2id: Dict[Any, int], num_classes: int) -> int:
    """
    Convert a manifest label (string or numeric) to a class id.
    """
    if lb in label2id:
...
def compute_class_weights(items: List[Dict[str, Any]], label2id: Dict[Any, int], num_classes: int) -> torch.Tensor:
    ...
    for it in items:
        lb = _get_item_label(it)
        y = _label_to_id(lb, label2id, num_classes)
        counts[y] += 1
...
def train_from_dict(
    ...
    if ptype == "predefined_split":
        split_key = str(protocol.get("split_key", "split"))
        train_value = str(protocol.get("train_value", "train"))
        val_value = str(protocol.get("val_value", "val"))
        test_value = str(protocol.get("test_value", "test"))

        tr_items = [it for it in items if str(it.get(split_key, "")).strip().lower() == train_value.lower()]
        va_items = [it for it in items if str(it.get(split_key, "")).strip().lower() == val_value.lower()]
        te_items = [it for it in items if str(it.get(split_key, "")).strip().lower() == test_value.lower()]

        if len(tr_items) == 0 or len(va_items) == 0 or len(te_items) == 0:
            raise ValueError(
                f"Empty predefined split detected: train={len(tr_items)}, val={len(va_items)}, test={len(te_items)}. "
                f"Check manifest field '{split_key}' and protocol values."
            )

    elif ptype == "repeated_holdout":
    ...
    base_seed = int(protocol.get("seed", 42))
    set_seed(base_seed + 10 + int(fold))
    ...
    for epoch in range(int(train_cfg["epochs"])):
        ...
        cur = float(val_out.get(metric_name, val_out["weighted_f1"]))
