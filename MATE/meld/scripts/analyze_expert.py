...
def _rebuild_split_from_cfg(items: List[Dict[str, Any]], cfg: Dict[str, Any], fold_id: int):
    """
    Fallback split reconstruction when splits.json is not available.
    Supports repeated_holdout and group_kfold.
    """
    ...
    if ptype == "predefined_split":
        ...
        tr = [it for it in items if str(it.get(split_key, "")).strip().lower() == train_value.lower()]
        va = [it for it in items if str(it.get(split_key, "")).strip().lower() == val_value.lower()]
        te = [it for it in items if str(it.get(split_key, "")).strip().lower() == test_value.lower()]

        if len(tr) == 0 or len(va) == 0 or len(te) == 0:
            raise ValueError(
                f"Empty predefined split detected: train={len(tr)}, val={len(va)}, test={len(te)}. "
