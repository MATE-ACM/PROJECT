...
"""Sequence model heads for HSEmotion video features."""
...
class VideoExperts(nn.Module):
    """
    Lightweight sequence heads:
      1) encoder=none + pool=attn_stats  -> ASP
      2) encoder=gru  + pool=attn_stats  -> BiGRU+Attn
      3) encoder=tcn_gn + pool=attn_stats -> TCN-GN
