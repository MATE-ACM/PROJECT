...
"""
Audio-Visual fusion expert: FiLM cross-modal conditioning over pooled stats.

Idea:
  - Pool each modality to stats vector pa,pv [B,2H]
  - Use pv to generate (gamma_a, beta_a) to modulate pa
  - Use pa to generate (gamma_v, beta_v) to modulate pv
  - Concat modulated vectors -> MLP -> logits

This often yields complementary behavior vs gated fusion.
"""
...
@register_expert("fusion_av_film")
class FusionAVFiLMExpert(nn.Module):
    """
    Config keys (expert)
    ...
    Recommended:
      ...
      - film_hidden (256)   # hidden size inside FiLM generators
      - mlp_dim (512)
    """
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # FiLM generators: from other modality pooled stats -> (gamma,beta) for target modality stats
        film_hidden = int(cfg.get("film_hidden", 256))
        out_dim = self.h * 2  # stats vector length
        self.gen_a = nn.Sequential(
            nn.Linear(out_dim, film_hidden),
            nn.GELU(),
            nn.Dropout(self.dropout),
            nn.Linear(film_hidden, out_dim * 2),
        )
        ...
        self.cls = nn.Sequential(
            nn.LayerNorm(out_dim * 2),  # concat of two modulated stats vectors
            nn.Dropout(self.dropout),
            nn.Linear(out_dim * 2, mlp_dim),
            nn.GELU(),
            nn.Dropout(self.dropout),
        ...
        self.last_attn: Optional[torch.Tensor] = None
        self.last_gamma_a: Optional[torch.Tensor] = None
        self.last_gamma_v: Optional[torch.Tensor] = None
    ...
    def forward_with_extras(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # pv conditions audio stats pa
        g_a, b_a = self.gen_a(pv).chunk(2, dim=-1)
        # keep scale stable: 1 + tanh(gamma)
        gamma_a = 1.0 + torch.tanh(g_a)
        pa_mod = gamma_a * pa + b_a

        # pa conditions video stats pv
        g_v, b_v = self.gen_v(pa).chunk(2, dim=-1)
        gamma_v = 1.0 + torch.tanh(g_v)
        pv_mod = gamma_v * pv + b_v

        fused = torch.cat([pa_mod, pv_mod], dim=-1)  # [B,4H]
        ...
        self.last_gamma_a = gamma_a
        self.last_gamma_v = gamma_v
