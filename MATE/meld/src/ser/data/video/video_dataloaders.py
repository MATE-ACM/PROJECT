...
from .video_npy import VideoNpyDataset, collate_video_npy
...
def make_video_loaders(
    ...
    max_frames = e.get("max_frames", 300)
    max_frames = int(max_frames) if max_frames is not None else None

    input_dim = e.get("input_dim")
    input_dim = int(input_dim) if input_dim is not None else None

    ext = str(e.get("ext", ".npz"))
    feat_key = str(e.get("feat_key", "x"))

    ds_tr = VideoNpyDataset(items_train, label2id=label2id, feat_root=str(feat_root),
                            ext=ext, feat_key=feat_key, pad_mode=pad_mode, max_frames=max_frames, input_dim=input_dim)
    ds_va = VideoNpyDataset(items_val,   label2id=label2id, feat_root=str(feat_root),
                            ext=ext, feat_key=feat_key, pad_mode=pad_mode, max_frames=max_frames, input_dim=input_dim)
    ds_te = VideoNpyDataset(items_test,  label2id=label2id, feat_root=str(feat_root),
                            ext=ext, feat_key=feat_key, pad_mode=pad_mode, max_frames=max_frames, input_dim=input_dim)

    collate_fn = partial(collate_video_npy, pad_mode=pad_mode, max_frames=max_frames)
    ...
    va = DataLoader(ds_va, batch_size=batch_size, shuffle=False,
                    num_workers=num_workers, collate_fn=collate_fn, drop_last=False,
                    pin_memory=pin_memory, persistent_workers=persistent_workers)
    te = DataLoader(ds_te, batch_size=batch_size, shuffle=False,
                    num_workers=num_workers, collate_fn=collate_fn, drop_last=False,
                    pin_memory=pin_memory, persistent_workers=persistent_workers)
    return tr, va, te
